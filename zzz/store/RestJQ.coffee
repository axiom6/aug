...
class RestJQ extends Store  # Rest with jQuery

  constructor:( stream, uri ) ->
    super( stream, uri, 'RestJQ' )
  ...
  ajaxRest:( op, t, id, object=null, params="" ) ->
    ...
    dataType  = @dataType()
    settings  = { url:url, type:@restOp(op), dataType:dataType, processData:false,
  ...
  ajaxSql:( op, t, where, objects=null, params="" ) ->
    ...
    settings.success = ( data,  status, jqXHR ) =>
      result = {}
      result = Util.toObjects(data,where,@key) if data?    and ( op is 'select' or op is 'remove' ) # toArray through
