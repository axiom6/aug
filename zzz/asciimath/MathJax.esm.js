...
(function (BASENAME) {
  ...
  let PROTO = [];  // a static object used to indicate when a prototype is being created
  let OBJECT = function (def) {
    let obj = def.constructor; if (!obj) {obj = function () {}}
    for (let id in def) {if (id !== 'constructor' && def.hasOwnProperty(id)) {obj[id] = def[id]}}
    return obj;
  };
  ...
  BASE.Object = OBJECT({
	...
	let dst = {};
	if (src != null) {
          for (let id in src) {if (src.hasOwnProperty(id)) {dst[id] = this.wrap(id,src[id])}}
	  // MSIE doesn't list toString even if it is not native so handle it separately
	...
	return dst;
      ...
      wrap: function (id,f) {
        ...
        let fn = function () {
          ...
          try { result = f.apply(this,arguments)} catch (err) {delete this.SUPER; throw err}
          delete this.SUPER;
          return result;
        }
...
(function (BASENAME) {
  let BASE = window[BASENAME];
  if (!BASE) {BASE = window[BASENAME] = {}}
  let isArray = BASE.Object.isArray;
  //
  //  Create a callback from an associative array
  //
  let CALLBACK = function (data) {
    let cb = function () {return arguments['callee']['execute'].apply(arguments['callee'],arguments)};
  ...
  //
  let EVAL = function (code) {return eval.call(window,code)}
  let TESTEVAL = function () {
    EVAL("let __TeSt_VaR__ = 1"); // check if it works in global context
    if (window.__TeSt_VaR__) {
      ...
      if (window['execScript']) {
        ...
        EVAL = function (code) {
          ...
          let result = BASE['__result']; delete BASE['__result']; delete BASE.__code;
        ...
        EVAL = function (code) {
          BASE.__code = code;
          code = "try {"+BASENAME+".__result = eval("+BASENAME+".__code)} catch(err) {"+BASENAME+".__result = err}";
          let head = (document.getElementsByTagName("head"))[0]; if (!head) {head = document.body}
  ...
  //
  let WAITFOR = function (callback,signal) {
    callback = USING(callback);
    if (!callback.called) {WAITSIGNAL(callback,signal); signal.pending++}
  };
  let WAITEXECUTE = function () {
    let signals = this.signal; delete this.signal;
    this.execute = this.oldExecute; delete this.oldExecute;
    let result = this.execute.apply(this,arguments);
    if (ISCALLBACK(result) && !result.called) {WAITSIGNAL(result,signals)} else {
      for (let i = 0, m = signals.length; i < m; i++) {
        signals[i].pending--;
        if (signals[i].pending <= 0) {signals[i].call()}
      }
    }
  };
  let WAITSIGNAL = function (callback,signals) {
  ...
  let AFTER = function (callback) {
    callback = USING(callback);
    callback.pending = 0;
    for (let i = 1, m = arguments.length; i < m; i++)
  ...
  //
  let HOOKS = MathJax.Object.Subclass({
    //
    ...
    Remove: function (hook) {
      for (let i = 0, m = this.hooks.length; i < m; i++) {
    ...
    Execute: function () {
      ...
      for (let i = 0, m = this.hooks.length; i < m; i++) {
        ...
        let result = this.hooks[i].apply(window,arguments);
    ...
    RemovePending: function () {
      ...
      for (let i = this.remove.length-1; i >= 0; i--) {this.hooks.splice(i,1)}
      this.remove = [];
    }
  ...
  //
  let EXECUTEHOOKS = function (hooks,data,reset) {
    if (!hooks) {return null}
    if (!isArray(hooks)) {hooks = [hooks]}
    if (!isArray(data))  {data = (data == null ? [] : [data])}
    let handler = HOOKS(reset);
    for (let i = 0, m = hooks.length; i < m; i++) {handler.Add(hooks[i])}
  ...
  //
  let QUEUE = BASE.Object.Subclass({
    //
    ...
    Process: function (queue) {
      while (!this.running && !this.pending && this.queue.length) {
        let callback = this.queue[0];
        queue = this.queue.slice(1); this.queue = [];
        this.Suspend(); let result = callback(); this.Resume();
  ...
  let SIGNAL = QUEUE.Subclass({
    ...
    Post: function (message,callback,forget) {
      ...
      if (this.posting || this.pending) {
        ...
        let result = this.listeners.Execute(message);
        if (ISCALLBACK(result) && !result.called) {WAITFOR(result,this)}
    ...
    Interest: function (callback,ignorePast,priority) {
      ...
      if (!ignorePast) {
        for (let i = 0, m = this.posted.length; i < m; i++) {
          callback.reset();
          let result = callback(this.posted[i]);
    ...
    ExecuteHooks: function (msg) {
      let type = (isArray(msg) ? msg[0] : msg);
...
(function (BASENAME) {
  ...
  let isSafari2 = (navigator.vendor === "Apple Computer, Inc." &&
                   typeof navigator.vendorSub === "undefined");
  let sheets = 0; // used by Safari2
  ...
  //
  let SCRIPTS = [];  // stores scripts to be removed after a delay
  let REMOVESCRIPTS = function () {
    for (let i = 0, m = SCRIPTS.length; i < m; i++) {BASE.Ajax.head.removeChild(SCRIPTS[i])}
    SCRIPTS = [];
  };
  ...
  BASE.Ajax = {
    ...
    fileName: function (url) {
      ...
      let recheck = false;
      do {
        for (let id in PATH) {if (PATH.hasOwnProperty(id) && PATH[id]) {
    ...
    fileRev: function (file) {
      let V = BASE.cdnFileVersions[file] || BASE.cdnVersion || '';
      if (V) {V = "?V="+V}
      return V;
    },
    ...
    Require: function (file,callback) {
      callback = BASE.Callback(callback); let type;
    ...
    LoadHook: function (file,callback,priority) {
      ...
      if (file instanceof Object)
        {for (let i in file) {if (file.hasOwnProperty(i)) {file = file[i]}}}
      file = this.fileURL(file);
    ...
    Preloading: function () {
      for (let i = 0, m = arguments.length; i < m; i++) {
        let file = this.fileURL(arguments[i]);
    ...
    loader: {
      ...
      JS: function (file,callback) {
        ...
        let script = document.createElement("script");
        let timeout = BASE.Callback(["loadTimeout",this,file]);
        this.loading[file] = {
          callback: callback,
    ...
    timer: {
      ...
      create: function (callback,node) {
        callback = BASE.Callback(callback);
        if (node.nodeName === "STYLE" && node.style &&
            typeof(node.style.cssText) !== 'undefined') {
      ...
      checkLength: function (check,node,callback) {
        if (check.time(callback)) return;
        let isStyle = 0; let sheet = (node.sheet || node.styleSheet);
    ...
    Styles: function (styles,callback) {
      let styleString = this.StyleString(styles);
      if (styleString === "") {
        callback = BASE.Callback(callback);
        callback();
      } else {
        ...
        if (style.style && typeof(style.style.cssText) !== 'undefined') {
          style.style.cssText = styleString;
        } else {
    ...
    StyleString: function (styles) {
      ...
      for (id in styles) {if (styles.hasOwnProperty(id)) {
        if (typeof styles[id] === 'string') {
          ...
          for (let i = 0; i < styles[id].length; i++) {
          ...
          style = [];
          for (let name in styles[id]) {if (styles[id].hasOwnProperty(name)) {
...
MathJax.HTML = {
  ...
  Element: function (type,def,contents) {
    let obj = document.createElement(type), id;
    if (def) {
      if (def.hasOwnProperty("style")) {
        let style = def.style; def.style = {};
  ...
  getScript: function (script) {
    let text = (script.text === "" ? script.innerHTML : script.text);
  ...
  Cookie: {
    ...
    Set: function (name,def) {
      let keys = [];
      if (def) {
        for (let id in def) {if (def.hasOwnProperty(id)) {
          keys.push(id+":"+def[id].toString().replace(/&/g,"&&"));
        }}
      }
      let cookie = this.prefix+"."+name+"="+escape(keys.join('&;'));
      if (this.expires) {
        let time = new Date(); time.setDate(time.getDate() + this.expires);
...
MathJax.Localization = {
  ...
  SPLIT: ("axb".split(/(x)/).length === 3 ?
    ...
    function (string,regex) {
      ...
      while ((match = regex.exec(string))) {
        result.push(string['substring'](last,match.index-last));
  ...
  processString: function (string,args,domain) {
    ...
    //
    let i, m, isArray = MathJax.Object.isArray;
    ...
    //
    let parts = this.SPLIT(string,this.pattern);
    for (i = 1, m = parts.length; i < m; i += 2) {
      let c = parts[i].charAt(0);  // first char will be { or \d or a char to be kept literally
      if (c >= "0" && c <= "9") {    // %n
        ...
        if (c >= "0" && c <= "9") {  // %{n}
          ...
          if (match) {
            if (match[1] === "plural") {
              let n = args[match[2]-1];
              if (typeof n === "undefined") {
                parts[i] = "???";        // argument doesn't exist
              } else {
                n = this.plural(n) - 1;  // index of the form to use
                let plurals = match[3].replace(/(^|[^%])(%%)*%\|/g,"$1$2%\uEFEF").split(/\|/); // the parts (replacing %| with a special character)
    ...
    for (let i = 0, m = snippet.length; i < m; i++) {
      if (MathJax.Object.isArray(snippet[i])) {
        ...
        if (typeof data[1] === "string") {        // [id,string,args]
          let id = data[0]; if (!MathJax.Object.isArray(id)) {id = [domain,id]}
          let phrase = this.lookupPhrase(id,data[1]);
  ...
  processMarkdown: function (phrase,args,domain) {
    ...
    //
    let parts = phrase.split(this.markdownPattern);
    let string = parts[0];
    //
    //  Loop through the matches and process them
    //
    for (let i = 1, m = parts.length; i < m; i += 8) {
  ...
  lookupPhrase: function (id,phrase,domain) {
    ...
    //
    let load = this.loadDomain(domain);
    ...
    //
    let localeData = this.strings[this.locale];
    if (localeData) {
      if (localeData.domains && domain in localeData.domains) {
        let domainData = localeData.domains[domain];
  ...
  loadFile: function (file,data,callback) {
    ...
    if (!file.match(/^([a-z]+:|\[MathJax\])/)) {
      let dir = (this.strings[this.locale].directory  ||
    ...
    //  over and over).
    //
    let load = MathJax.Ajax.Require(file,function () {data.isLoaded = true; return callback()});
    //
  ...
  loadDomain: function (domain,callback) {
    let load, localeData = this.strings[this.locale];
    if (localeData) {
  ...
  resetLocale: function(locale) {
    ...
    let remap = this.strings[locale]['remap'];
  ...
  addTranslation: function (locale,domain,definition) {
    let data = this.strings[locale], isNew = false;
  ...
  //
  setCSS: function (div) {
    let locale = this.strings[this.locale];
    if (locale) {
...
MathJax.Message = {
  ...
  MoveFrame: function () {
    let body = (MathJax.Message.quirks ? document.body : document.documentElement);
    let frame = MathJax.Message.frame;
  ...
  Log: function () {
    let strings = [];
    for (let i = 1, m = this.log.length; i < m; i++) {strings[i] = this.log[i].text}
    return strings.join("\n");
  }
...
MathJax.Hub = {
  ...
  CombineConfig: function (name,def) {
    let config = this.config, id, parent; name = name.split(/\./);
    for (let i = 0, m = name.length; i < m; i++) {
  ...
  getAllJax: function (element) {
    let jax = [], scripts = this.elementScripts(element);
  ...
  Typeset: function (element,callback) {
    ...
    let ec = this.elementCallback(element,callback);
    let queue = {};
    if (ec.count) {
  ...
  PreProcess: function (element,callback) {
    let ec = this.elementCallback(element,callback);
    let queue = MathJax.Callback.Queue();
    if (ec.count) {
      let elements = (ec.count === 1 ? [ec.elements] : ec.elements);
  ...
  takeAction: function (action,element,callback) {
    let ec = this.elementCallback(element,callback);
    let elements = ec.elements;
    let queue = MathJax.Callback.Queue(["Clear",this.signal]);
    let state = {
    ...
    if (ec.count) {
      let delay = ["Delay",MathJax.Callback,this.processSectionDelay];
      if (!delay[2]) {delay = {}}
      queue.Push(
  ...
  scriptAction: {
    ...
    Update: function (script) {
      let jax = script.MathJax.elementJax;
  ...
  prepareScripts: function (action,element,state) {
    ...
    let scripts = this.elementScripts(element);
    let STATE = MathJax.ElementJax.STATE;
    for (let i = 0, m = scripts.length; i < m; i++) {
  ...
  checkScriptSiblings: function (script) {
    ...
    let config = this.config, pre = script.previousSibling;
    if (pre && pre.nodeName === "#text") {
      let preJax,postJax, post = script.nextSibling;
      ...
      if ( post && postJax && (!config.preJax || preJax)) {
  ...
  processInput: function (state) {
    let jax, STATE = MathJax.ElementJax.STATE;
    let script, prev, m = state.scripts.length;
    try {
      //
      ...
      while (state.i < m) {
        ...
        if (!script.MathJax.elementJax || script.MathJax.state === STATE.UPDATE) {
          ...
          let type = script.type.replace(/ *;(.|\s)*/,"");  // the input jax type
          let input = this.inputJax[type];                  // the input jax itself
        ...
        //
        state.i++; let now = new Date().getTime();
  ...
  prepareOutput: function (state,method) {
    while (state.j < state.jaxIDs.length) {
      let id = state.jaxIDs[state.j], JAX = MathJax.OutputJax[id];
      if (JAX[method]) {
        try {
          let result = JAX[method](state);
  ...
  processOutput: function (state) {
    let result, STATE = MathJax.ElementJax.STATE, script, m = state.scripts.length;
    try {
      //
      //  Loop through the scripts
      //
      while (state.i < m) {
        ...
        let jax = script.MathJax.elementJax; if (!jax) {state.i++; continue}
        //
  ...
  processMessage: function (state,type) {
    let m = Math.floor(state.i/(state.scripts.length)*100);
    let message = (type === "Output" ? ["TypesetMath","Typesetting math: %1%%"] :
  ...
  formatError: function (script,err) {
    let LOCALIZE = function (id,text,arg1,arg2) {return MathJax.Localization._(id,text,arg1,arg2)};
    //
    //  Get the error message, URL, and line, and save it for
    //    reporting in the Show Math As Error menu
    //
    let message = LOCALIZE("ErrorMessage","Error: %1",err.message)+"\n";
    ...
    //
    let errorSettings = this.config.errorSettings;
    let errorText = LOCALIZE(errorSettings.messageId,errorSettings.message);
    let error = MathJax.HTML.Element("span", {
    ...
    MathJax.Ajax.Require("[MathJax]/extensions/MathEvents.js",function () {
      let EVENT = MathJax.Extension['MathEvents'].Event,
          HUB = MathJax.Hub;
    ...
    //
    let node = document.getElementById(error.id);
  ...
  elementCallback: function (element,callback) {
    ...
    for (let i = 0, m = element.length; i < m; i++)
  ...
  HTMLCollection2Array: function (nodes) {
    ...
    let NODES = [];
    for (let i = 0, m = nodes.length; i < m; i++) {NODES[i] = nodes[i]}
    return NODES;
  },

  Insert: function (dst,src) {
    for (let id in src) {if (src.hasOwnProperty(id)) {
...
MathJax.Hub.Startup = {
  ...
  Config: function () {
    ...
    if (this.params.config) {
      let files = this.params.config.split(/,/);
  ...
  ConfigDelay: function () {
    let delay = this.params.delayStartupUntil || MathJax.Hub.config.delayStartupUntil;
  ...
  ConfigBlocks: function () {
    ...
    let queue = MathJax.Callback.Queue();
    for (let i = 0, m = scripts.length; i < m; i++) {
      let type = String(scripts[i].type).replace(/ /g,"");
  ...
  Cookie: function () {
    return this.queue.Push(
      ...
      [function (config) {
        let SETTINGS = config.menuSettings;
        if (SETTINGS.locale) MathJax.Localization.resetLocale(SETTINGS.locale);
        let renderer = config.menuSettings.renderer, jax = config.jax;
        if (renderer) {
          let name = "output/"+renderer; jax.sort();
  ...
  //
  Jax: function () {
    let config = MathJax.Hub.config, jax = MathJax.Hub.outputJax;
    //  Save the order of the output jax since they are loading asynchronously
    for (let i = 0, m = config.jax.length, k = 0; i < m; i++) {
      let name = config.jax[i].substr(7);
  ...
  //
  Menu: function () {
    let menu = MathJax.Hub.config.menuSettings, jax = MathJax.Hub.outputJax, registered;
    for (let id in jax) {if (jax.hasOwnProperty(id)) {
  ...
  Hash: function () {
    if (MathJax.Hub.config.positionToHash && document.location.hash &&
      ...
      let name = decodeURIComponent(document.location.hash.substr(1));
  ...
  HashCheck: function (target) {
    let jax = MathJax.Hub.getJaxFor(target);
  ...
  loadArray: function (files,dir,name,synchronous) {
    if (files) {
      ...
      if (files.length) {
        let queue = MathJax.Callback.Queue(), callback = {}, file;
        for (let i = 0, m = files.length; i < m; i++) {
...
(function (BASENAME) {
  let BASE = window[BASENAME], ROOT = "["+BASENAME+"]";
  let HUB = BASE.Hub, AJAX = BASE.Ajax, CALLBACK = BASE.Callback;

  let JAX = MathJax.Object.Subclass({
    JAXFILE: "jax.js",
    ...
    Augment: function (def,cdef) {
      let cObject = this.constructor, ndef = {};
      if (def != null) {
        for (let id in def) {if (def.hasOwnProperty(id)) {
  ...
  BASE.InputJax = JAX.Subclass({
    ...
    copyTranslate: true,
    Process: function () {
      let queue = CALLBACK.Queue(), file;
      // Load any needed element jax
      let jax = this.elementJax; if (!BASE.Object.isArray(jax)) {jax = [jax]}
      for (let i = 0, m = jax.length; i < m; i++) {
      ...
      let load = queue.Push(AJAX.Require(file));
      if (!load.called) {
  ...
  BASE.OutputJax = JAX.Subclass({
    copyTranslate: true,
    preProcess: function () {
      let load, file = this.directory+"/"+this.JAXFILE;
    ...
    Process: function () {throw Error(this.id + " output jax failed to load properly")},
    Register: function (mimetype) {
      let jax = HUB.outputJax;
  ...
  BASE.ElementJax = JAX.Subclass({
    ...
    Detach: function () {
      let script = this.SourceElement(); if (!script) return;
    ...
    Subclass: function () {
      let obj = JAX.Subclass.apply(this,arguments);
  ...
  BASE.OutputJax.Error = {
    ...
    Jax: function (text,script) {
      let jax = MathJax.Hub.inputJax[script.type.replace(/ *;(.|\s)*/,"")];
      this.errors++;
      return {
...
/**********************************************************/
  ...
  (function (BASENAME) {
    ...
    let HUB = BASE.Hub; let STARTUP = HUB.Startup; let CONFIG = HUB.config;
    let HEAD = document.head || (document.getElementsByTagName("head")[0]);
    if (!HEAD) {HEAD = document.childNodes[0]}
    let scripts = (document.documentElement || document).getElementsByTagName("script");
    if (scripts.length === 0 && HEAD.namespaceURI)
      scripts = document.getElementsByTagNameNS(HEAD.namespaceURI,"script");
    let namePattern = new RegExp("(^|/)"+BASENAME+"\\.js(\\?.*)?$");
    for (let i = scripts.length-1; i >= 0; i--) {
      if ((scripts[i].src||"").match(namePattern)) {
        STARTUP.script = scripts[i].innerHTML;
        if (RegExp.$2) {
          let params = RegExp.$2.substr(1).split(/&/);
          for (let j = 0, m = params.length; j < m; j++) {
            let KV = params[j].match(/(.*)=(.*)/);
    ...
    let BROWSERS = {
      ...
      versionAtLeast: function (v) {
        let bv = (this.version).split('.'); v = v.split('.');
        for (let i = 0, m = v.length; i < m; i++)
        {if (bv[i] !== v[i]) {return parseInt(bv[i]||"0") >= parseInt(v[i])}}
        return true;
      },
      Select: function (choices) {
        let browser = choices[HUB.Browser];
    ...
    HUB.Browser = HUB.Insert(HUB.Insert("Unknown"),{version: "0.0"});

    for (let browser in BROWSERS) {if (BROWSERS.hasOwnProperty(browser)) {
      if (BROWSERS[browser] && browser.substring(0,2) === "is") {
        browser = browser.slice(2);
        if (browser === "Mac" || browser === "PC") continue;
        HUB.Browser = HUB.Insert(browser,BROWSERS);
        ...
        let MATCH = VERSION.exec(xAGENT) || ["","","","unknown","0.0"];
    ...
    try {HUB.Browser.Select({
      Safari: function (browser) {
        let v = parseInt((String(browser.version).split("."))[0]);
      ...
      Firefox: function (browser) {
        if ((browser.version === "0.0" || AGENT.match(/Firefox/) == null) &&
          ...
          let rv = AGENT.match(/[\/ ]rv:(\d+\.\d.*?)[) ]/);
          if (rv) {browser.version = rv[1]}
          else {
            let date = (navigator['buildID']||navigator.productSub||"0").substr(0,8);
        ...
        if (document['documentMode'] < 10 && !STARTUP.params['NoMathPlayer']) {
          ...
          try {
            if (browser.hasMathPlayer) {
              let mathplayer = document.createElement("object");
