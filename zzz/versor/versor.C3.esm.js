...
C3.Ro = {
	...
	dls: function(x, y, z, r) {
		let s = C3.Ro.point(x, y, z);
		if(r > 0) s[4] -= .5 * (r * r);
		else s[4] += .5 * (r * r);
        return s;
	},
	circle: function(cen, dir, r) {
		let plane = cen.op(C3.dual(dir))
		let sphere = C3.Ro.dls(cen[0], cen[1], cen[2], r);
	...
	split: function(pp) {
		...
		let dlp = C3.e5(-1).ip(pp);
		let bstA = C3['Bst'](pp);
		let bstB = C3['Bst'](pp);
		bstA[0] -= r;
		bstB[0] += r;
		let pA = C3['Pnt'](bstA.div(dlp));
		let pB = C3['Pnt'](bstB.div(dlp));
		return [pA, pB];
	},
};
...
let feq = function(v1, v2, eps) {
	return Math.abs(v1-v2) <= eps;
}

C3.Gen = {
	rot: function(b) {
		let c = Math.sqrt(-C3.wt(b));
		let sc = Math.sin(-c);
		if(Math.abs(c) > 1e-7) {
			sc /= c;
		}
	...
	mot: function(dll) {
		...
		let c = Math.sqrt(Math.abs(wt))
		let cc = Math.cos(c);
		let sc = Math.sin(c);

		let Bu = C3.unit(Bv);
		let t = C3['Vec3'](dll[3], dll[4], dll[5]);

		let tv = C3.Op.pj(t, Bu);
		let tw = C3.Op.rj(t, Bu);

		tv = tv.gp(sinc(c));

		let tt = tw.gp(cc).add(tv);
		let ts = Bv.gp(tw);

		return C3['Mot'](cc, Bu[0]*sc, Bu[1]*sc, Bu[2]*sc, tt[0], tt[1], tt[2], ts[3]*sc);
	},
	log: function(m) {
		let ac = Math.acos(m[0]);
		let den = sinc(ac);
		let den2 = ac*ac*den;
		...
		if(feq(den2, 0, 1e-8)) {
			...
			let b = C3['Biv3'](C3.Ori.ip(m.gp(C3.Inf)).gp(-1/den));
			let tq = C3.Dll(b.gp(m));

			// perpendicular (along line of axis)
			let cperp = C3.Drv(b.gp(C3['Drt'](m[7])).gp(-1/den2));
			// parallel (in plane of rotation)
			let cpara = C3.Drv(b.gp(tq).gp(-1/den2));

			let c = cperp.add(cpara);
    ...
    ratio: function(a, b, t) {
		...
		let dll = C3.Gen.log(m).gp(t*0.5);
		if(feq(C3.norm(dll), 0, 1e-8)) {
			let ac = Math.acos(m[0]);
			let PAO = C3.Ro.point(0, 0, 0)
			let p1 = C3.Fl.loc(a, PAO);
			let p2 = C3.Fl.loc(b, PAO);
			let trans = p2.sub(p1);

			if(feq(ac, Math.PI, 1e-8)) {
				let biv = C3.Biv3(a);
				let within = C3.Op.pj(C3.Vec3(1, 0, 0), biv);
				...
				let rotBiv = C3.duale(within).gp(t);
				trans = trans.gp(t);

				let rot = C3.Rot(rotBiv);
				rot[0] = 1-t;
