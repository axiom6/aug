...
Chord = class Chord {
  ...
  ready() {
    ...
    this.chord = this.d3.chord().padAngle(0.05).sortSubgroups(this.d3.descending);
    this.arc = this.d3.arc().innerRadius(this.inner).outerRadius(this.outer);
    this.ribbon = this.d3.ribbon().radius(this.inner);
    this.color = this.d3.scaleOrdinal().domain(this.d3.range(4)).range(this.range);
