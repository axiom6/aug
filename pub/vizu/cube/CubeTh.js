...
CubeTh = class CubeTh {
  ...
  constructor(build1, parId, guiFlag1, mix) {
    var elems;
    ...
    if (this.debug) {
      console.log('Cameras.Cube()', {
        ...
        scenePos: this.scene.position
      });
    }
    // @scene.add(@camera)
    this.verify(this.scene);
    this.verify(this.camera);
  ...
  subscribe(stream) {
    ...
    this.stream.subscribe('Select', 'Cube', (topic) => {
  ...
  onSelect(topic) {
    ...
    switch (topic.name) {
      ...
      case 'Share':
        ...
        break;
      default:
        console.error('CubeTh.onSelect()', topic);
    }
  }
  ...
  resizeScreen() {
    ...
    if (this.debug) {
      console.log("CubeTh.resizeScreen", {
  ...
  traverse(prop, name, act) {
    ...
    reveal = (child) => {
      ...
      if (this.debug) {
        return console.log('CubeTh.traverse.end', {
          name: child.name,
          geom: child.geom,
          prop: prop,
          value: value,
  ...
  verify(object3D) {
    ...
    for (i = 0, len = props.length; i < len; i++) {
      ...
      for (n in obj) {
        ...
        console.log('CubeTH.verify() name', {
          klass: klass,
          prop: prop,
          type: obj,
          n: n,
          v: v,
          incsNaN: this.arrayHasNaN(v)
        });
        if ((!this.mix.isArray(v) && isNaN(v)) || (n === 'elements' && this.arrayHasNaN(v))) {
          console.log('CubeTH.verify() NaN ', {
            klass: klass,
            prop: prop,
            type: obj,
            n: n,
