  ...
  <div class="comp" ref="Comp" title="Comp">
    ...
    <template v-for="prac in practices">
      <div v-show="isPrac(prac.name)" :class="pracDir(prac.dir)" :key="prac.name" :ref="prac.name" :title="prac.name">
        <p-icon :style="style" :comp="comp" :prac="prac"></p-icon>
...
<script type="module">
  ...
  let Comp = {
    ...
    data() { return {
      comp:'None', prac:'All', disp:'All', style:{ display:'none' }, practices:{},
      pages:[
    ...
    methods: {
      stPage: function (page) {
        this.style = this.page === page ? { display:'grid' } : { display:'none' }; },
