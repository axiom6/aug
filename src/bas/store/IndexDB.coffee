...
class IndexDB
  ...
  txnObjectStore:( table, mode ) -> # , key=@key
    txo = null
    if not @db?
      console.trace( 'Store.IndexedDb.txnObjectStore() @db null', @dbName )
    else if @db.objectStoreNames.contains(table)
      txn = @db.transaction( table, mode )# if @txnUp? then @txnUp else @db.transaction( table, mode )
  ...
  deleteDB:( dbName ) ->
    ...
    request.onsuccess = () =>
      console.log(   'Index.deleteDB() deleted',           { dbName:dbName } )
    request.onerror   = () =>
      console.error( 'Index.deleteDB() unable to delete',  { dbName:dbName, error:request.error } )
    request.onblocked = () =>
      console.error( 'Index.deleteDB() database blocked', { dbName:dbName, error:request.error } )
    return  
