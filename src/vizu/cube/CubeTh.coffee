...
class CubeTh
  ...
  constructor:( @build, @parId, @guiFlag, @mix ) ->
    ...
    console.log( 'Cameras.Cube()',
      { camera:@camera, camPos:@camera.position, scene:@scene, renderer:@renderer, scenePos:@scene.position } ) if @debug
    # @scene.add(@camera)
    @verify( @scene  )
    @verify( @camera )
  ...
  onSelect:( topic ) =>
    console.log( 'CubeTh.onCube()', name ) if @debug
    switch   topic.name
  ...
  resizeScreen:() =>
    ...
    if @camera? and @renderer?
       ...
       @renderer.setSize( @screenWidth, @screenHeight )
    console.log( "CubeTh.resizeScreen", { width:@screenWidth, height:@screenHeight } )  if @debug
  ...
  traverse:( prop, name, act ) =>
    ...
    console.log( 'CubeTh.traverse()', { prop:prop, name:name, visible:act[name] } )  if @debug
    reveal = (child) =>
      console.log( 'CubeTh.traverse.beg',
        { name:child['name'], tprop:prop, cprop:child[prop], value:value, visible:child.visible } ) if @debug
      if child[prop]? and child[prop] is name
         child.visible = act[name]
      console.log( 'CubeTh.traverse.end', { name:child.name, geom:child.geom, prop:prop, value:value, visible:child.visible } ) if @debug
  ...
  verify:( object3D ) ->
    ...
    props  = ["matrix"]   # "position","rotation","scale", "matrixWorld","modelViewMatrix","normalMatrix","quaternion"]
    names  = ["elements"] # "x","_x","y","_y","z","_z","w","_w",
    for prop in props
      obj = object3D[prop]
      for n,v of obj when @mix.inArray(n,names)
        console.log(   'CubeTH.verify() name', { klass:klass, prop:prop, type:obj, n:n, v:v, incsNaN:@arrayHasNaN(v) } )
        if ( not @mix.isArray(v) and isNaN(v) ) or ( n is 'elements' and @arrayHasNaN(v) )
          console.log( 'CubeTH.verify() NaN ', { klass:klass, prop:prop, type:obj, n:n, v:v } )
    return
