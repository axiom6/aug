...
test().describe( "", "Initial run through of 23 tests" )
test( "isEnums(arg,oper,type)",  spec.isEnums(enumStr,oper,type),     true    )
test( "verifySpec(arg)",         spec.verifySpec(specObj),           specObj  )
#est( "conditions(args...)",     spec.conditions(args),              specObj  )  # Tested by @verifySpec(specObj)
...
test( "toEnums(arg)",                                 spec.toEnums(enumStr), ["Embrace","Innovate","Encourage"] )
...
test( "inEnums(result,spec,status,level,key,index)",  spec.inEnums(resultArray, rangeSpec, status), true )
test( "inRange(result,spec,status,level,key,index)",  spec.inRange("Embrace",   enumsSpec, status), true )
