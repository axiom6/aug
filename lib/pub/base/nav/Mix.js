...
Mix = class Mix {
  ...
  inArray(e, a) {
    return t.inArray(e, a);
  }

  inObject(key, obj) {
    return t.inObject(key, obj);
  }

  isChild(key) {
    return t.isChild(key, obj);
  }
  ...
  inovObject(compKey, inovKey) {
    ...
    if (this.isBatch(compKey)) {
      ...
      if (t.isDef(inovKey) && inovKey !== compKey && this.isBatch(inovKey)) {
  ...
  removeElem(elem) {
    while (t.isDef(elem) && t.isDef(elem.firstChild)) {
