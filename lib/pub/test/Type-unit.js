var a, abc, args, b, c, func, i, j, k, l, len, len1, len2, len3, len4, m, o, stream, streamLog, subjects, t, toConvertArgs, toKlassArgs, toStrArgs, toTypeArgs, toValueArgs, undef;
...
for (i = 0, len = toTypeArgs.length; i < len; i++) {
  args = toTypeArgs[i];
  test(args);
}
...
for (j = 0, len1 = toKlassArgs.length; j < len1; j++) {
  args = toKlassArgs[j];
  test(args);
}
...
for (k = 0, len2 = toConvertArgs.length; k < len2; k++) {
  args = toConvertArgs[k];
  test(args);
}
...
for (l = 0, len3 = toValueArgs.length; l < len3; l++) {
  args = toValueArgs[l];
  test(args);
}
...
toStrArgs = [
  ...
  [
    ...
    "b",
    "c"],
    "\'[\"a\",\"b\",\"c\"]\'" // Backslashes allow us to see what was generated
  ],
  [
    {
      a: "x",
    ...
    "\'{a:\"x\",b:\"y\"}\'" // Backslashes allow us to see what was generated
  ],
  [BigInt(123),
  123]
];

for (m = 0, len4 = toStrArgs.length; m < len4; m++) {
  args = toStrArgs[m];
  test(args);
}
