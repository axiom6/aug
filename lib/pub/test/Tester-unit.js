var Main, add, five, func, stream, streamLog, subjects, undef;
...
test("Tester.five() = 5", function(t) {
  return t.eq(five(), 5);
});

test("Tester.add(2,3) = 5", function(t) {
...
unit("Tester.type({a:'a'})", tester.type({
  a: 'a'
}), 'object');
...
unit("Tester.klass({a:'a'})", tester.klass({
  a: 'a'
}), 'Object');
...
unit("Tester.isNaN(NaN)", tester.isNaN(0/0), true);

unit("Tester.isObj({a:'a'})", tester.isObj({
  a: 'a'
}), true);
...
unit("Tester.isNot({a:'a'}", tester.isNot({
  a: 'a'
}), false);
...
unit("Tester.isVal({a:'a'})", tester.isVal({
  a: 'a'
}), false);
...
unit("Tester.isArray({a:'a'})", tester.isArray({
  a: 'a'
}), true);

test('Tester.eq( {a:"a"})', function(t) {
  return t.eq({
    a: "a"
  }, {
    a: "a"
...
test("Tester.eq([1,2,3])", function(t) {
...
test('Tester.eq( (x) ->, (y) -> ) )', function(t) {
  return t.eq(function(x) {}, function(y) {});
});

// String conversions
unit('Tester.toStr({ a:"a", b:"b" })', tester.toStr({
  a: "a",
  b: "b"
}), '{ a:"a", b:"b" }');
