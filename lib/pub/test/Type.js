...
Type = class Type {
  ...
  isNone(s) {
    return this.isType(s, "string") && s === "none";
  }
  ...
  isDef(d) {
    return !this.isIn(this.toType(d), "undefs");
  }
  ...
  */
  ...
  inside(str) {
    return this.slice(str, 2, str.length - 1);
  }

  // First onvert \' to \" then cnclose an arg in single quotes when double quotes are inside
  toSingleQuote(arg) {
    var has, str;
    str = !this.isStr(arg) ? this.toStr(arg, 0, true) : arg;
    str = str.includes("'") ? str.replaceAll("'", '"') : str; // Needs to be more precise
    has = str.includes('"');
    str = has ? this.toEnclose("'", str, "'") : str;
    if (this.debug) {
      console.log("Type.toSingleQuote(arg)", {
        arg: arg,
        str: str,
        has: has
      });
      ...
      case "string":
        switch (arg) {
          ...
          case "false":
            return false;
          default:
            return this.toWarn("toBoolean(arg)", "unable to convert", arg, "boolean", false, function(t) {
              return t.log(t.warn());
            });
        }
        break;
  ...
  toArray(arg) {
    var array, j, key, len1, obj, value, values;
  ...
  isInTypeKeyArgs(key, args) {
    ...
    for (j = 0, len1 = args.length; j < len1; j++) {
      arg = args[j];
      isn = isn && this.isInTypeEnum(key, arg);
    }
    return isn;
  }
