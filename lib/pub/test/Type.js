...
Type = class Type {
  ...
  */
  ...
  toValue(str, encloseValue = false) {
    var val;
    if (!this.isStr(str)) {
      val = this.toStr(str, encloseValue);
      if (this.debug) {
    ...
    str = (function() {
      switch (type) {
        ...
        case "boolean":
          ...
          break;
        case "array":
          return this.toStrArray(arg, encloseValue);
        case "object":
          return this.toStrObject(arg, encloseValue, encloseKey);
        case "null":
          return "null";
        case "undefined":
          return "undefined";
  ...
  toStringify(arg) {
    return this.toStr(arg, true, true);
  }
  ...
  toStrObject(obj, encloseValue = false, encloseKey = false) {
    ...
    for (key in obj) {
      ...
      val = obj[key];
      okey = encloseKey ? '"' + key(+'"') : key;
      str += okey + ":" + this.toStr(val, encloseValue, encloseKey) + ",";
    }
  ...
  toStrArray(array, encloseValue = false) {
    ...
    for (i = j = 0, ref = array.length; (0 <= ref ? j < ref : j > ref); i = 0 <= ref ? ++j : --j) {
      str += this.toStr(array[i], encloseValue);
  ...
  v(val) {
    var ret;
    ret = this.isType(val, 'string') ? this.toStr(val, true) : this.toValue(val, true);
    if (this.debug) {
      this.log({
        v: "v",
        ret: ret,
