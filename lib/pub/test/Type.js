...
Type = class Type {
  ...
  toArray(arg) {
    ...
    switch (this.type(arg)) {
      ...
      case "string":
        arg = this.strip(arg, "[", "]");
        array = [];
        splits = this.arg.split(",");
        for (j = 0, len1 = splits.length; j < len1; j++) {
          split = splits[j];
          array.push(this.toValue(split));
        }
        return array;
      default:
        return [];
  ...
  isEnums(arg) {
    switch (false) {
      case !(this.isStr(arg) && arg.includes("|")):
        return true;
  ...
  // Useless
  listFunctions(obj) {
    var fun, key, list, par;
    list = [];
    console.log(obj);
    for (key in obj) {
      if (!hasProp.call(obj, key)) continue;
      fun = obj[key];
      if (!(this.isType(fun, "function"))) {
        continue;
      }
      par = fun.toString();
      list.push({
  ...
  parseFunction(par) {
    return (par + '').replace(/[\/][\/].*$/mg, '').replace(/\s+/g, '').replace(/[\/][*][^\/*]*[*][\/]/g, '').split('){', 1)[0].replace(/^[^(]*[(]/, '').replace(/=[^,]+/g, '').split(',').filter(Boolean); // strip single-line comments // strip white space // strip multi-line comments // extract the parameters // strip any ES6 defaults // split & filter [""]
  }

  listFunctions1(obj) {
    var fun, key, list, proto, str;
    list = [];
    for (key in obj) {
      if (!hasProp.call(obj, key)) continue;
      fun = obj[key];
  ...
  listFunctions2(obj) {
    var arg, fun, j, key, len1, list, ref, val;
    list = [];
    for (key in obj) {
      if (!hasProp.call(obj, key)) continue;
      val = obj[key];
  ...
  listFunctions3(obj) {
    var j, key, len1, list, props;
    list = [];
    props = Object.getOwnPropertyNames(obj);
    for (j = 0, len1 = props.length; j < len1; j++) {
      key = props[j];
      if (!(this.isType(obj[key], "function"))) {
        continue;
      }
      list.push(key);
      console.log(key);
