  ...
  */
  ...
  toStr(arg, keyValue = "") {
    ...
    str = (function() {
      switch (type) {
        ...
        case "range":
        case "any":
          if (this.inEnums("value", keyValue)) {
            return '"' + arg(+'"');
          } else {
            return arg;
          }
        ...
        case "boolean":
          ...
          break;
        case "array":
          return this.toStrArray(arg, keyValue);
        case "object":
          return this.toStrObject(arg, keyValue);
        case "null":
          return "null";
        case "undefined":
          return "undefined";
  ...
  toStrObject(obj, keyValue = "") {
    ...
    for (key in obj) {
      ...
      val = obj[key];
      okey = this.inEnums("key", keyValue) ? '"' + key(+'"') : key;
      str += okey + ":" + this.toStr(val, keyValue) + ",";
    }
  ...
  toStrArray(array, keyValue = "") {
    ...
    for (i = j = 0, ref = array.length; (0 <= ref ? j < ref : j > ref); i = 0 <= ref ? ++j : --j) {
      str += this.toStr(array[i], keyValue);
  ...
  v(val) {
    var ret;
    ret = this.isType(val, 'string') ? this.toStr(val, "value") : this.toValue(val);
    this.log({
      v: "v",
      ret: ret,
      val: val,
