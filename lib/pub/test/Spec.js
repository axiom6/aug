...
Spec = class Spec extends Type {
  ...
  isRegex(arg, oper) {
    return oper === "regex" && this.isType(arg, "string");
  }
    ...
    return spec = (function() {
      switch (false) {
        case !this.isSpecParse(expect, type):
          return this.toSpecParse(spec, type);
        case !this.isSpecObject(expect, type):
          return this.toSpecObject(spec, type);
        default:
  ...
  isSpecParse(arg, type) {
    return type === "regex" || (type === "string" && arg.includes(":"));
  }
  ...
  toSpecParse(spec, arg) {
    ...
    if (length >= 1) { // expect
      spec.spec = splits[1];
      if (this.isType(splits[1], "regex")) { // regex
        spec.oper = "regex";
...
Spec.specs = [
  "range",
  "enums",
  "regex" // high level spec   based comparision specs
];
