...
Spec = class Spec extends Type {
  ...
  inSpec(result, spec) {
    ...
    if (!(this.isSpec(spec) && this.type(result) === spec.type && this.inCard(result, spec))) {
      return false;
    }
    match = spec.match;
    switch (false) {
  ...
  inRange(result, range) {
    ...
    if (this.isRanges(range)) {
      return this.inRanges(result, range);
    }
    if (!this.isRange(range)) {
      return false;
    }
    a = this.toRangeArray(range); // Convers the 'string' represention of a if necessary
    ...
    switch (false) {
      ...
      case !this.isArray(results):
        for (k = 0, len = results.length; k < len; k++) {
          result = results[k];
          pass = pass && this.inRange(result, ranges);
        }
        break;
      default:
        pass = false;
