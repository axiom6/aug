...
Spec = class Spec extends Type {
  ...
  isIn(type, arg) {
    ...
    } else if (this.isEnumsArg(arg)) {
      return this.inArray(type, this.toEnums(arg));
    } else if (this.isStr(arg)) {
      if (Type[arg] != null) {
        return Type[arg].includes(type);
      } else if (Spec[arg] != null) {
        return Spec[arg].includes(type);
      } else {
        return this.isWarn(false, `arg ${arg} missing for`, type, false);
      }
    } else {
