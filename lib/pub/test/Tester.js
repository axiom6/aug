...
Tester = class Tester extends Type {
  ...
  examine(pass, result, expect, status, warn, key, index) {
    ...
    status.assert.text = prefix + this.module + "." + status.assert.text;
  ...
  report(result, expect, status) {
    ...
    this.statusText = this.toStatusText();
    ...
    this.summedClear = false;
    ({
      toStatusText: function() {
        if (this.isStr(this.methodId)) {
          return "\n" + this.methodId + this.enclose(status.assert.text, "()") + " ";
        } else {
          return "\n" + status.assert.text + " ";
        }
      }
  ...
  textResult(result, key = null, index = null) {
    ...
    return `Result${ref}where type is ${this.type(result)} and value is ${this.toStr(result)}`;
  }
  ...
  textExpect(expect, key = null, index = null) {
    ...
    return `Expect${ref}where type is ${this.type(expect)} and value is ${this.toStr(expect)}`;
  }
  ...
  static module(moduleId, moduleTx, moduleOn = true) {
    Tester.moduleId = moduleId;
    Tester.moduleTx = moduleTx;
    Tester.moduleOn = moduleOn;
    return Tester;
  }
  ...
  complete() {
    ...
    summaryText = this.block(); // Prepend any block statuses
