...
Tester = class Tester extends Spec {
  constructor() {
    ...
    this.isObjFuncArgs = this.isObjFuncArgs.bind(this);
    ...
    this.f = null;
    this.a = null;
    this.arg0 = ""; // set by test() that is passed inside eq() and sent to run()
    this.modulePaths = {};
  ...
  test(...args) {
    ...
    } else if (args.length === 2 && this.isObjFuncArgs(args[0]) && !this.isFunction(args[1])) {
      // @log( "test()", {args:args, isArgs:@isObjFuncArgs(args[0]) } ) if @debug
      this.arg0 = args[0];
      result = this.applyFuncArgs(this.arg0);
      expect = args[1];
    } else if (args.length === 3 && !this.isFunction(args[1])) {
      this.arg0 = args[0];
      result = args[1];
      expect = args[2];
      this.run(this.arg0, result, expect); // returns tester instance for chaining
    }
    return this;
  }
  ...
  exam(args) {
    ...
    return this.test(arg0, expect);
  }

  isObjFuncArgs(arg0) {
    boundMethodCheck(this, Tester);
    return this.isObject(arg0) && (arg0.o != null) && this.isType(arg0.o, "object") && (arg0.f != null) && this.isType(arg0.f, "function") && (arg0.a != null) && this.isType(arg0.a, "array");
  }

  applyFuncArgs(q) {
    var a, as, f, fs, o, os, r, rs;
    o = q.o != null ? q.o : this.o;
    f = q.f != null ? q.f : this.f;
    a = q.a != null ? q.a : [];
    r = af.apply(o, a);
    rs = this.toStr(r);
    os = this.toStr(o.constructor.name);
    fs = f.name();
    as = this.toStr(a);
    this.log("apply", `${rs}=${os}.${fs}(${as})`);
    return r;
  }
  ...
  eq(result, expect) {
    boundMethodCheck(this, Tester);
    return this.run(this.arg0, result, expect);
  }

  // -- run() scenario is @initStatus(...) @assert(...)
  run(arg0, result, expect) {
    this.statusAs = this.initStatus(result, expect, arg0);
  ...
  verify(result, expect, status) {
    ...
    if (this.isStr(status.errors.text)) {
      console.log("Tester.verify(result,expect,status)", {
  ...
  statusAssertText(pass, result, status) {
    ...
    if (this.isNot(result)) {
      ...
      if (this.debug) {
        console.log("Tester.statusAssertText()", {
          arg0: this.arg0
        });
      }
      text += this.strip(describeName, "", "()") + "(" + this.toStr(this.arg0) + ") ";
    } else {
      text += this.arg0 + " ";
    }
    return text;
