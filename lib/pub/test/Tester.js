...
Tester = class Tester extends Spec {
  ...
  testingOff(methodOn = this.methodOn) {
    return !(this.always || (this.testing && this.moduleOn && methodOn));
  }
  ...
  initStatus(result, expect, text) {
    return {
      assert: {
        ...
        methodTx: this.methodTx,
        methodId: this.methodId,
        methodOn: this.methodOn,
        methodOp: this.methodOp
      },
      warned: {
        text: ""
      },
  ...
  assert(result, expect, status, level = 0, key = null, index = null) {
    ...
    status = this.checkValuesTypes(result, expect, status);
    if (!status.assert.pass) {
      return this.examine(false, result, expect, status, key, index);
    }
      ...
      status = (function() {
        switch (spec.oper) {
          ...
          default:
            ...
            return this.examine(false, result, spec, status, key, index);
        }
      }).call(this);
      return status;
    }
  ...
  convert(result, expect, status) {
    status = this.checkValuesTypes(result, expect, status);
    return this.examine(status.assert.pass, result, expect, status);
  }
  ...
  inRegexp(result, spec, status, level = 0, key = null, index = null) {
    ...
    return this.examine(pass, result, spec, status, key, index);
  }
  ...
  async runUnitTests(paths) {
    ...
    for (j = 0, len = paths.length; j < len; j++) {
      ...
      this.complete(); // This where we know that the unit test module has finished so generate a module summary
    }
  ...
  toPath(path) {
    var dirs;
    dirs = path.split("/");
    this.moduleId = this.tail(dirs).split("-")[0];
    this.modulePaths[this.moduleId] = {
      name: this.moduleId,
      path: path
    };
    if (this.debug) {
    ...
    return this.modulePaths[this.moduleId];
  }

  module(moduleTx, moduleId = "") {
    boundMethodCheck(this, Tester);
    this.moduleTx = moduleTx;
    this.moduleId = this.isStr(moduleId) ? moduleId : this.isStr(this.moduleId) ? this.moduleId : "";
    return this;
  }
  ...
  statusAssertText(pass, result, status) {
    ...
    text = pass ? "\n-- Passed -- " : "\n-- Failed -- ";
    if (this.isStr(methodId) && this.head(methodId) !== "-") {
      text += this.strip(methodId, "", "()") + "(" + this.toStr(result) + ") ";
  ...
  isGroup(status, group, pass = null) {
    ...
    switch (group) {
      case "method":
        return passed(status, pass) && this.methodId === status.assert.methodId;
      case "module":
        return passed(status, pass) && this.moduleId === status.assert.moduleId;
      case "all":
  ...
  summary() {
    var summaryText;
    if (this.testingOff()) { // blank string turns off logging
      return "";
    }
    summaryText = this.title("method", "Summary");
    summaryText += this.summaryText("method");
    summaryText += this.totals("method");
    if (this.isDef(this.stream)) {
      this.stream.publish(this.summarySubject, summaryText);
    }
    this.reset(); // reset all @method..  parameters
  ...
  complete(arg = null) {
    ...
    this.reset("module"); // reset all @method..  and @module.. parameters
    return this;
  }
