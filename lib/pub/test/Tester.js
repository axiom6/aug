...
Tester = class Tester extends Type {
  ...
  summary(module, method) {
    var all, fail, failCount, fullCount, j, k, len, len1, pass, passCount, path, ref1, ref2, summaryText;
    ...
    summaryText = "";
    if (!all && module === this.module) { // module method summary
      passCount = 0;
      failCount = 0;
      ref1 = this.passed;
  ...
  toSchema(expect) {
    ...
    return schema = (function() {
      switch (false) {
        ...
        default:
          return this.toInfo("toSchema(expect)", "expect not schema 'string' or 'object'", expect, type, "schema", this.toStr(schema), schema, function(t) {
            return t.log(t.info());
          });
      }
    }).call(this);
  ...
  inRange(result, schema, status, level, key, index) {
    ...
    pass = (function() {
      switch (type) {
        ...
        default:
          return this.toInfo("inRange()", "unknown range type", result, type, type, "false", false, function(t) {
            return t.log(t.info());
          });
      }
    }).call(this);
  ...
  inArrayRange(result, range) {
    ...
    if (nRange === 1) {
      ...
      pass = this.toInfo("inRange()", text, result, type, type, "false", false, function(t) {
        return t.log(t.info());
      });
    } else if (nResult < nRange) {
      text = `OK with more range bounds ${nRange} than needed for result ${nResult}`;
      pass = this.toInfo("inRange()", result, text, type, type, "true", true, function(t) {
        return t.log(t.info());
      });
  ...
  toEnums(arg) {
    ...
    switch (type) {
      ...
      case "array":
        ...
        break;
      default:
        enums = this.toInfo("toEnums(arg)", "unable to convert", arg, type, "enums", "[]", [], function(t) {
          return t.log(t.info());
        });
    }
    return enums;
  ...
  isRange(range) {
    ...
    switch (type) {
      ...
      default:
        return this.toInfo("isRange(range)", "not a range type", range, type, "", "false", false, function(t) {
          return t.log(t.info());
        });
    }
  }
