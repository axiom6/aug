...
Tester = (function() {
  class Tester {
      ...
      if (status.assert.pass) {
        status = (function() {
          switch (this.type(result)) {
            ...
            case "number":
            case "boolean":
              return this.arguesEq(result, expect, status, op);
            case "object":
    ...
    examine(pass, result, expect, status, op, info, key, index) {
      ...
      status.expect.text += this.textValue("Expect", eType, argue, key, index);
      return status;
    }

    // Convert expect to a schema object if op is schema
    isSchema(v) {
      return (v.op != null) && (v.type != null) && (v.argue != null) && (v.range != null) && (v.op != null) && (v.size != null);
    }
    ...
    toSchema(expect, op) {
      ...
      switch (this.type(expect)) {
        ...
        case "object":
          ...
          schema.argue = expect.argue != null ? expect.argue : "any";
    ...
    parseSchema(expect, schema) {
      ...
      if (length >= 1) {
        if (splits[1][0] === "[") {
          ...
          schema.ranges = "any";
          schema.argue = this.toType(splits[1], schema.type);
        }
    ...
    arguesEq(result, expect, status, op) {
      var argue, i, length, oper, pass, ranges;
      argue = expect;
      if (op === "schema") {
        argue = expect.argue;
    ...
    textValue(name, argue, key, index) {
      ...
      return `${name}${ref}where type is ${this.type(argue)} and argue is ${this.toString(argue)}`;
    }
    ...
    arraysEq(result, expect, status, op, level) {
      var argue, i, info, j, length, ref1;
      argue = expect;
      ...
      if (op === "schema") {
        ...
        if (argue === "any") {
          ...
          return status;
        } else if (argue.size !== "any" && result.length > argue.size) {
        ...
        } else if (!this.isArray(argue)) {
          return status;
        }
      }
      // Examine the array lengths
      if (result.length !== argue.length) {
    ...
    toString(arg, enc = "") {
      ...
      switch (type) {
        ...
        case "array": // This combination of travesal and recursion is cleaner than JSON.stringify()
          ...
          break;
        case "function":
          this.toInfo("toString(arg)", arg, "function", "string", "?function?");
          break;
        case "null":
          "null";
          break;
        ...
        case "symbol":
          ...
          break;
        default:
          this.toInfo("toString(arg)", arg, type, "string", arg.toString()); // return of arg.toString() is a hail mary
      }
      ...
      switch (type) {
        ...
        case "string":
          if (this.isStringFloat(arg)) {
            ...
            return this.toInfo("toFloat(arg)", arg, "string", "float", 0/0);
          }
          break;
        default:
          return this.toInfo("toFloat(arg)", arg, type, "float", 0/0);
      }
    }
      ...
      switch (type) {
        ...
        case "string":
          if (this.isStringInt(arg)) {
            ...
            return this.toInfo("toInt(arg)", arg, "string", "int", 0/0);
          }
          break;
        default:
          return this.toInfo("toInt(arg)", arg, type, "int", 0/0);
      }
    }

    toInfo(method, arg, type, typeTo, retn) {
      this.info += `\n  Tester.${method} unable to convert ${this.toString(arg)} of '${type}' to'${typeTo}' returning '${this.toString(retn)}'`;
      return retn;
    }
        ...
        case "string":
          switch (arg) {
            ...
            case "false":
              return false;
            default:
              return this.toInfo("toBoolean(arg)", arg, type, "boolean", false);
          }
          break;
        case "int":
    ...
    toRange(arg) {
      if (this.isRange(arg)) {
        ...
        return [0, arg[0]];
      } else if (this.isType(arg, 'int')) {
        return [0, arg];
      } else if (this.isType(arg, 'float')) {
        return [0, this.toInt(arg)];
      } else {
        this.info += `\n  Tester.toRange(arg) unable to convert ${this.toString(arg)} to an 'int' Range`;
        return [0, 100, 0];
      }
    }
