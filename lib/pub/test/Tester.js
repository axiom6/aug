...
Tester = class Tester extends Type {
  ...
  test(text, ...args) {
    ...
    if (arguments.length === 0 || !(this.testing && this.moduleOn && this.methodOn)) {
      return this;
  ...
  examine(pass, result, expect, status, warn, key, index) {
    ...
    status.assert.text = pass ? "-- Passed -- " : "-- Failed -- ";
  ...
  report(result, expect, status) {
    ...
    this.statusText = this.toStatusText(status, result);
    ...
    this.summedText += this.statusText;
    return this.summedClear = false;
  }

  toStatusText(status, result) {
    if (this.isStr(this.methodId)) {
      return "\n" + status.assert.text + this.methodId + this.toEnclose(result, "()");
    } else {
      return "\n" + status.assert.text;
    }
  }
  ...
  textResult(result, key = null, index = null) {
    ...
    return `Result${ref} type is '${this.type(result)}' with value ${this.toStr(result)}`;
  }
  ...
  textExpect(expect, key = null, index = null) {
    ...
    return `Expect${ref} type is '${this.type(expect)}' with value ${this.toStr(expect)}`;
  }
  ...
  textSchema(schema, key = null, index = null) {
    ...
    return `Schema${ref} type is '${schema.type}' with spec '${schema.spec}' and oper '${schema.oper}'`;
  }
  ...
  summary() {
    var fail, failCount, fullCount, j, k, len, len1, methodOn, pass, passCount, path, ref1, ref2, summaryText;
    methodOn = this.methodOn; // Remember method on status
    this.methoOn = true;
    path = (typeof module !== "undefined" && module !== null) && (this.modules[module] != null) ? this.modules[module].path : "?";
    passCount = 0;
    failCount = 0;
    ref1 = this.passed;
    for (j = 0, len = ref1.length; j < len; j++) {
      pass = ref1[j];
      if (pass.assert.module === this.moduleId) {
        ++passCount;
      }
    }
    ref2 = this.failed;
    for (k = 0, len1 = ref2.length; k < len1; k++) {
      fail = ref2[k];
      if (fail.assert.module === this.moduleId) {
        ++failCount;
      }
    }
    fullCount = passCount + failCount;
    summaryText = `\nSummary for ${this.methodId} ${this.methodTx}`;
    summaryText += this.summedText; // Prepend #summedText from accumulated statuses
    summaryText += `\n-- Totals -- for ${this.moduleId} in ${path}`;
    ...
    this.methodOn = true;
    this.summedClear = true;
    if (this.isDef(this.stream)) { // Need to consider the role of each subject
    ...
    if (methodOn) {
      ...
      return ""; // a blank string turns off logging
    }
  }

  complete() {
    var fullCount, summaryText;
    boundMethodCheck(this, Tester);
    fullCount = this.passed.length + this.failed.length;
    summaryText = this.summedText; // Prepend any block statuses
  ...
  isResultType(type) {
    ...
    return this.isInfo(pass, "Not a Result", type, Type.results, function(t) {
      return t.log(t.info());
    });
  }
  ...
  isExpect(expect, oper) {
    ...
    pass = (function() {
      switch (oper) {
        ...
        default:
          return this.isInfo(false, "Not a Expect oper", oper, Type.opers, function(t) {
            return t.log(t.info());
          });
      }
    }).call(this);
  ...
  isExpectType(type) {
    ...
    return this.isInfo(pass, "Not a Expect", type, Tester.expects, function(t) {
      return t.log(t.info());
    });
  }
  ...
  isOper(oper) {
    ...
    return this.isInfo(pass, "Not an 'oper'", oper, "opers", function(t) {
      return t.log(t.info());
    });
  }
  ...
  isCard(card) {
    ...
    return this.isInfo(pass, "Not a 'card'", card, Type.cards, function(t) {
      return t.log(t.info());
    });
  }
  ...
  isSchema(expect) {
    ...
    type = this.type(expect);
    isParse = this.isSchemaParse(expect, type);
    isObj = this.isSchemaObject(expect, type);
    if (this.debug) {
      console.log("isSchema(expect)", {
        expect: expect,
        type: type,
        isParse: isParse,
        isObj: isObj
  ...
  toSchema(expect) {
    ...
    return schema = (function() {
      switch (false) {
        ...
        default:
          return this.toWarn("toSchema(expect)", "expect not schema 'string' or 'object'", expect, type, "schema", schema, function(t) {
            return t.log(t.info());
          });
      }
    }).call(this);
  ...
  isIn(type, key) {
    if (Type[key] != null) {
      ...
      return this.isInfo(false, `key ${key} missing for`, type, [], function(t) {
        return t.log(t.info());
      });
    }
  }
