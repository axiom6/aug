...
Tester = class Tester extends Spec {
  constructor() {
    ...
    this.isArgums = this.isArgums.bind(this);
    this.toArgums = this.toArgums.bind(this);
  ...
  test(...args) {
    ...
    this.argums = args.length >= 1 ? this.toArgums(args[0]) : {
      has: false,
      arg0: "none"
    };
    if (args.length === 2 && this.isFunction(args[1])) {
      closure = args[1];
      closure(this); // Call closure with an injected tester instance
    } else if (args.length === 2 && this.argums.has && !this.isFunction(args[1])) {
      this.log("test()", {
        args: args,
        argums: this.argums // if @debug
      });
  ...
  exam(args) {
    ...
    return this.test(args, expect);
  }

  isArgums(argums) {
    boundMethodCheck(this, Tester);
    return this.isArray(argums) || this.isObject(argums) && (argums.obj != null) && this.isType(argums.obj, "object") && (argums.func != null) && this.isType(argums.func, "function") && (argums.args != null) && this.isType(argums.args, "array");
  }
  ...
  toArgums(arg0) {
    ...
    snap.arg0 = "none";
    snap.obj = arg0.obj != null ? arg0.obj : this.argums.obj;
    snap.func = arg0.func != null ? arg0.func : this.argums.func;
    snap.args = (function() {
      switch (false) {
        case !this.isArray(arg0):
          return arg0;
        case !this.isObject(arg0):
          return arg0.args;
        default:
          return "none";
      }
    ...
    snap.arg0 = snap.args;
    snap.has = this.isArgums(snap);
    if (snap.has) {
      this.argums.obj = snap.obj; // Remember the lastest verified arg0 settings
      this.argums.func = snap.func;
      this.argums.args = snap.args;
      this.argums.has = snap.has;
    }
    this.log("toArgums()", snap); // if @debug
    return snap;
  }
  ...
  applyArgums(argums) {
    var result;
    argums.obj = argums.obj != null ? argums.obj : this.o;
    argums.func = argums.func != null ? argums.func : this.f;
    argums.args = argums.args != null ? argums.args : [];
    result = argums.func.apply(argums.obj, argums.args);
    this.log("apply", this.toArgumsStr(result, argums));
    return result;
  }

  // result = obj.func( args... )
  toArgumsStr(result, argums) {
    var argsStr, funcStr, objStr, resulStr;
    resulStr = this.toStr(result);
    ...
    return `${resulStr}=${objStr}.${funcStr}(${argsStr})`;
  }
  ...
  assert(result, expect, status, level = 0, key = null, index = null) {
    ...
    if (!status.assert.pass) {
      ...
      return this.processStatus(status, level);
    }
  ...
  statusAssertText(pass, result, status) {
    var arg0, describeName, text;
    if (status.argums.has) {
      this.log("statusAssertText", status.argums); // if @debug
      return this.toArgumsStr(result, status.argums);
    }
    arg0 = status.argums.arg0;
    describeName = status.assert.describeName;
    if (this.debug) {
      this.log("Tester.statusAssertText()", {
        arg0: arg0
      });
    }
    text = pass ? "\n-- Passed -- " : "\n-- Failed -- ";
    if (this.isNot(result)) {
      text += this.textValue("Result", result);
    } else if (this.isStr(describeName)) {
      text += this.strip(describeName, "", "()") + "(" + this.toStr(arg0) + ") ";
    } else {
      text += arg0 + " ";
    }
    return text;
