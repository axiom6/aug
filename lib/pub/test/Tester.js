...
Tester = (function() {
  class Tester {
    ...
    checkValuesTypes(result, expect, op, status, key, index) {
      ...
      info = (function() {
        switch (false) {
          ...
          case !(rType !== eType && eType !== "range" && eType !== "schema"):
    ...
    isSchema(v) {
      (v.cardinality != null) && (v.type != null) && (v.value != null) && (v.range != null) && (v.oper != null);
      // Cardinality ? = optional 1 = required * = 0 to many + = 1 to many min:max is a range
      return Tester.cardinalityEnums = ["?", "1", "*", "+", "min:max"];
    }
    ...
    toSchema(expect, op) {
      ...
      switch (typeExpect) {
        ...
        case "floatRange":
          ...
          schema.type = op; // ???
          schema.value = expect;
          schema.range = this.isArray(expect) ? expect : ["any"]; // ???
          schema.oper = op;
          break;
        case "string":
          ...
          break;
        case "object":
          schema.cardinaly = expect.opt === "?" ? expect.opt : "1"; // "1" implies key required
          schema.type = this.isString(expect.type) ? expect.type : "any";
          schema.value = expect.value != null ? expect.value : "any";
          schema.range = this.isArray(expect.range) ? expect.ranges : ["any"];
      }
      return schema;
    }
    ...
    parseSchema(expect, schema) {
      ...
      if (length >= 1) {
        if (splits[1][0] === "[") {
          schema.ranges = this.parseRange(splits[1]);
          schema.value = "any";
    ...
    parseRange(splits) {
      var array, j, l, len1, len2, len3, len4, p, q, range, split, str, strs;
      range = [];
            ...
            for (l = 0, len2 = splits.length; l < len2; l++) {
              ...
              if (this.head(split, "[[", false) === "[[" && this.tail(split, "]]", false) === "]]") {
                ...
                for (p = 0, len3 = strs.length; p < len3; p++) {
                  str = strs[p];
                  range.push[this.toArray(str)];
                }
                return range;
              } else if (this.head(split) === "[" && this.tail(split) === "]") {
                range.push[this.toArray(split)];
                return range;
              }
            }
            break;
    ...
    inMyRange(result, range) {
      ...
      rangeType = this.type(result);
      if (!pass || resultType(iant(rangeType))) {
        return this.toInfo("inRange(result,range)", result, resultType, rangeType, "false", false);
      }
