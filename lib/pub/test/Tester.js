...
Tester = class Tester extends Type {
  ...
  statusAssertText(pass, status, result) {
    var text;
    text = pass ? "\n-- Passed -- " : "\n-- Failed -- ";
    if (this.isStr(this.methodId) && this.methodId.charAt(0) !== "-") {
      text += this.methodId + "(" + this.toStr(result) + ")";
    }
    return text;
  }
  ...
  examine(pass, result, expect, status, warn, key, index) {
    ...
    if (isSchema) {
      status.expect.text += this.textValue("Schema", expect, key, index);
    }
  ...
  totals(group) {
    var failCount, fullCount, j, k, len, len1, passCount, ref1, ref2, status, text;
    passCount = 0;
    failCount = 0;
    fullCount = this.statuses.length;
    ref1 = this.statuses;
    for (j = 0, len = ref1.length; j < len; j++) {
      status = ref1[j];
      if (this.isGroup(status, group, true)) {
        ++passCount;
      }
    }
    ref2 = this.statuses;
    for (k = 0, len1 = ref2.length; k < len1; k++) {
      status = ref2[k];
      if (this.isGroup(status, group, false)) {
        ++failCount;
      }
    }
    text = this.totalsTitle(group);
  ...
  totalsTitle(group) {
    var path, title;
    path = group === "module" && (this.modules[group] != null) ? this.modules[group].path : "";
    title = "\nTotals for ";
    title += (function() {
      switch (group) {
        case "method":
  ...
  isExpect(expect, oper) {
    ...
    pass = (function() {
      switch (oper) {
        ...
        default:
          return this.isWarn(false, "Not a Expect oper", oper, Type.opers, function(t) {
            return t.log(t.warn());
          });
      }
    }).call(this);
    return this.isWarn(pass, "Not a Expect", type, Type.expects);
  }

  isExpectType(type) {
    var pass;
    pass = this.isDef(type) && this.isIn(type, "expects");
    return this.isWarn(pass, "Not a Expect", type, Tester.expects, function(t) {
      return t.log(t.warn());
    });
  }
  ...
  isCard(card) {
    ...
    return this.isWarn(pass, "Not a 'card'", card, Type.cards, function(t) {
      return t.log(t.warn());
    });
  }
