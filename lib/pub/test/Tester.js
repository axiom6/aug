...
Tester = class Tester extends Type {
  constructor() {
    ...
    this.always = true; // When true  all testing is turned on  which overrides all other settings
  ...
  setOptions(options) {
    ...
    this.always = options.always != null ? options.always : true;
  ...
  statusAssertText(pass, result) {
    ...
    if (this.isStr(this.methodId) && this.methodId.charAt(0) !== "-" && this.tail(this.methodId) === ")") {
      text += this.strip(this.methodId, "", "()") + "(" + this.toStr(result) + ") " + this.text;
    } else {
      text += this.text;
    }
    return text;
