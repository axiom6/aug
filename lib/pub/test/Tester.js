...
Tester = class Tester {
  ...
  func(closure) {
    ...
    this.code = tokens[1] != null ? tokens[1].substring(0, tokens[1].length - 3) : "";
    if (this.logToConsole && this.verbose) {
      return console.log(this.code, closure, str, tokens[1]);
    }
  }
  ...
  report(status) {
    ...
    this.statusMsg = `${status.assert.text} ${eq} ${status.expect.value}`;
    if (this.verbose || !status.assert.pass) {
      this.statusMsg += `"\n  ${status.result.text} ${status.result.value}`;
    }
    if (this.verbose || !status.assert.pass) {
      this.statusMsg += `"\n  ${status.expect.text} ${status.expect.value}`;
    }
    if (this.isStr(this.code) && (this.verbose || !status.assert.pass)) {
      this.statusMsg += "\n" + this.code;
    }
    console.log(this.statusMsg); // if @called is "test" or ( @logToConsole and not @sent )
    this.sent = false;
    this.code = "";
    this.called = "";
  }
  ...
  klass(val) {
    var typ;
    typ = this.type(val, false); // Uppercase to screen for Null and Undefined
    switch (typ) {
      case 'Null':
        return 'Null';
      case 'Undefined':
